...
public static class NoiseMapGenerator
{
    ...
    public static float[,] GenerateFallOffMap(int size, AnimationCurve ac)
    {
        ...
        for (int i = 0; i < size; i++) for (int j = 0; j < size; j++)// for 
            {
                //          CALCULATE x AS (i / size) * 2 - 1 
                //     CALCULATE y AS(j / size) * 2 – 1
                float x = i / (float)size * 2 - 1;
                float y = j / (float)size * 2 - 1;

                float value = Mathf.Max(Mathf.Abs(x), Mathf.Abs(y));

                falloffMap[i, j] = Evaluate(  value); // this uses the eval math constant.

             //   falloffMap[i, j] = ac.Evaluate(value); // uses Unity curve in the inspector
    ...
    static float Evaluate(float val) { float a = 3f, b = 2.2f; return Mathf.Pow(val, a) / (Mathf.Pow(val, a) + Mathf.Pow(b - b * val, a)); }
